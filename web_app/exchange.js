...
const exchange_name = 'Harambe Ex';             // TODO: fill in the name of your exchange

const token_name = 'Harambe';                // TODO: replace with name of your token
const token_symbol = 'HAR';              // TODO: replace with symbol for your token
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    var balances = await exchange_contract.connect(provider.getSigner(defaultAccount)).getExchangeRate();
    var exchange_rate = balances[0]/balances[1]; // check indexes 


    var max_exchange_rate = exchange_rate * (1 + maxSlippagePct/100);
    var min_exchange_rate = exchange_rate * (1 - maxSlippagePct/100);
    console.log(max_exchange_rate,min_exchange_rate)
    //console.log(ethers.utils.parseUnits(()).toString());
    console.log(ethers.utils.parseUnits((amountEth*10**18).toString(),"wei"));
    await exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity(max_exchange_rate,min_exchange_rate,{ value: ethers.utils.parseUnits("1",'wei') });
    //(amountEth*10**18).toString()
    //var deposit = await exchange_contract.connect(exchange_address).addLiquidity(max_exchange_rate,min_exchange_rate,{ value: ethers.utils.parseUnits((amountEth*10**18).toString(),'wei') });
...
async function removeLiquidity(amountEth, maxSlippagePct) {
  ...
  var remove = await exchange_contract.removeLiquidity(max_exchange_rate,min_exchange_rate);
...
async function removeAllLiquidity(maxSlippagePct) {
  ...
  var remove = await exchange_contract.removeAllLiquidity(max_exchange_rate,min_exchange_rate);
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  var swap = await exchange_contract.swapTokensForETH(amountToken, max_exchange_rate); 
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  var max_exchange_rate = exchange_rate * (1 + maxSlippagePct/100);
  //figure amountETH -> msg.value 
  var swap = await exchange_contract.swapTokensForETH(max_exchange_rate); 
